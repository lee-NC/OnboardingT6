...
namespace Demo.Services.UserService.Entity.Api.Entities;
...
public class UserPassCredential : BaseCredential
{
    ...
    [BsonElement("passHash")] public byte[]? PasswordHash { get; set; }

    [BsonElement("passSalt")] public byte[]? PasswordSalt { get; set; }

    [BsonElement("accessFailedCount")] public int AccessFailedCount { get; set; }

    [BsonElement("lockedDate")] public DateTime? LockedDate { get; set; }

    [BsonElement("isLocked")] public bool IsLocked { get; set; }
    //Trạng thái tài khoản đang bị khóa đăng nhập (sẽ tự mở sau 5 phút từ khi bị khóa)

    public bool IsLocking => IsLocked && LockedDate.HasValue && LockedDate.Value.AddMinutes(5) > DateTime.UtcNow;
}