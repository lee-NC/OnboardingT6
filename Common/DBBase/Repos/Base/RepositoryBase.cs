...
public abstract class RepositoryBase<T> : IRepositoryBase<T> where T : EntityBase, new()
{
    ...
    public T FindAsNoTracking(Guid id)
    {
        return Table.Where(x => id.Equals(x.Id)).AsNoTracking().FirstOrDefault();
    }

    public T FindIgnoreQueryFilters(Guid id)
    {
        return Table.IgnoreQueryFilters().FirstOrDefault(x => id.Equals(x.Id));
    }
