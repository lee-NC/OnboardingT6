...
public static class MongoCollectionQueryByPageExtensions
{
        ...
        int pageSize)
    {
        ...
        if (sortDefinition != null) agg.AppendStage(PipelineStageDefinitionBuilder.Sort(sortDefinition));
        //defs = new[]
        //{
        ...
        //PipelineStageDefinitionBuilder.Limit<TDocument>(pageSize),
        //};
        if (projectionDef != null) agg.AppendStage(PipelineStageDefinitionBuilder.Project(projectionDef));

        var dataFacet = AggregateFacet.Create("data", agg);

        var aggregateOptions = new AggregateOptions();
        if (sortDefinition != null) aggregateOptions = new AggregateOptions { AllowDiskUse = true };
