...
public static class EntityUtils
{
    ...
    public static byte[] GetInputPasswordHash(string pwd, byte[] salt)
    {
        ...
        var inputPwdHasher = new Rfc2898DeriveBytes(inputPwdBytes, salt, PasswordGenerator.PasswordDerivationIteration,
    ...
    public static bool VerifyPassword(byte[] oldPwdHash, string newPassword, byte[] passwordSalt)
    {
        ...
        for (var i = 0; i < PasswordGenerator.PasswordSaltLength; i++) salt[i] = passwordSalt[i];

        var newPwdHash = GetInputPasswordHash(newPassword, salt);

        if (newPwdHash.SequenceEqual(oldPwdHash)) return true;
