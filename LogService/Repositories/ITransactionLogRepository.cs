...
public interface ITransactionLogRepository : IMongoDbRepositoryBase<TransactionLog>
    {
        ...
        Task<(int TotalPages, long TotalCount, IReadOnlyList<TransactionLog> Items)>
            FindAll(string customerId, int status, int page, int pageSize, string orderBy, bool isDesc, DateTime startDate, DateTime endDate);
        Task<(int TotalPages, long TotalCount, IReadOnlyList<TransactionLog> Items)>
            FindAllBySearch(string textSearch, int status, int page, int pageSize, string orderBy, bool isDesc, DateTime startDate, DateTime endDate);

        Task<List<TransactionLog>> FindByExistFields(Dictionary<string, object> param);
        Task<(int PageIndex, int PageSize, int TotalPages, long TotalCount, IReadOnlyList<TransactionLog> Items)> FindByCustomerId(string customerId, int page, int pageSize);
